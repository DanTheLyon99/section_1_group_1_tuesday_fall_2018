...
void* idleTimer(void *para){
    double secondsPassed = difftime(timeNow,timeStart);  
    while(isIdle){      
        time(&timeNow);


        if( difftime(timeNow,timeStart) - secondsPassed >= IDLE_TIME){   
            clearScreen();
            ...
            secondsPassed = difftime(timeNow,timeStart);  
            Sleep(1000);    
        }
...
void play(double timeAlreadyPassed,int localInitialOpen){
    ...
    if (r>12)
        y = 35+2*(r-12);
    char statement[16] = {"Mode "};
    char sizes[10];
    sprintf(sizes,"%d, %d",x,y);
    strcat(statement,sizes);     
    system(statement);
